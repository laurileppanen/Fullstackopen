...
test('URL, likes and user are displayed when the view button is clicked', async () => {
    ...
    const component = render(
      <Blog blog={blog} toggleLikes={mockHandler} handleDelete={mockHandler} userLoggedIn="late22" />
    )
    ...
    expect(component.container.querySelector('.moredetails')).toHaveStyle('display: none')
  
    await user.click(button)
  
    expect(component.container.querySelector('.moredetails')).not.toHaveStyle('display: none')
